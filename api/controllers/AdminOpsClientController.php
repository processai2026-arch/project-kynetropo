 ...
 * DELETE /admin/ops/clients/{id}         — delete with its projects (OpsRecordRemoval)
 */
class AdminOpsClientController
{
    ...
    public function index(Request $request): void
    {
        ...
        $sql    = "SELECT c.*, p.name AS project_name, p.id AS project_id, p.project_code,
    ...
    public function store(Request $request): void
    {
        ...
        try {
            $code = trim((string)($body['client_code'] ?? '')) !== ''
                ? OpsCodes::accept($tenantId, OpsCodes::CLIENT, $body['client_code'])
                : OpsCodes::next($tenantId, OpsCodes::CLIENT);
    ...
    public function update(Request $request): void
    {
        ...
        if (array_key_exists('client_code', $body) && strtoupper(trim((string)$body['client_code'])) !== (string)($client['client_code'] ?? '')) {
            try {
                $updates['client_code'] = OpsCodes::accept($tenantId, OpsCodes::CLIENT, $body['client_code'], $id);
            } catch (AppException $e) {
                Response::error($e->getMessage(), $e->getCode());
            }
        }
    ...
    public function destroy(Request $request): void
    {
        ...
        $tenantId = Database::tenantId();
        try {
            $removed = OpsRecordRemoval::deleteClient($tenantId, $id);
        } catch (AppException $e) {
            Response::error($e->getMessage(), $e->getCode());
        }
        Response::success($removed, 'Client deleted');
    }
