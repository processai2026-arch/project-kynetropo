...
export interface OpsProject {
  id: number;
  /** Project ID shown in the CRM (PRJ-0001). Auto-numbered oldest first, editable. */
  project_code: string | null;
  client_id: number;
  client_code: string | null;
...
/** GET /admin/ops/projects/{id}: the project with everything its page shows. */
export interface OpsProjectDetail extends OpsProject {
  stage_history: OpsProjectStage[];
  bugs: OpsBug[];
  meetings: OpsMeeting[];
  payments: OpsPayment[];
  activity_log: OpsActivityEntry[];
}
...
export interface OpsBug {
  ...
  created_at: string;
  updated_at: string;
  screenshots?: { id: number; file_path: string; uploaded_by?: string | null }[];
