...
export const opsClientsApi = {
  ...
  /** Permanently deletes the client with its projects, payments, meetings and bugs. */
...
export const opsBugsApi = {
  ...
  addComment: (id: number, comment: string, added_by: string) =>
    apiFetch<{ data: unknown }>(`/admin/ops/bugs/${id}/comments`, { method: "POST", body: JSON.stringify({ comment, added_by }) }),
...
export const opsFinanceApi = {
  ...
  updatePayment:  (id: number, body: Partial<OpsPayment>) => apiFetch<{ data: OpsPayment }>(`/admin/ops/finance/payments/${id}`, { method: "PUT", body: JSON.stringify(body) }),
